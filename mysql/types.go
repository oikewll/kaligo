...
// Date is the struct for MySQL DATE type
type Date struct {
    Year       int16
    Month, Day byte
}
...
// IsZero is the function for true if date is 0000-00-00
func (dd Date) IsZero() bool {
    return dd.Day == 0 && dd.Month == 0 && dd.Year == 0
}

// Time is the function for converts Date to time.Time using loc location.
...
// Localtime is the function for converts Date to time.Time using Local location.
// Converts MySQL zero to time.Time zero.
func (dd Date) Localtime() time.Time {
    return dd.Time(time.Local)
}

// ParseDate is the function for convert string date in format YYYY-MM-DD to Date.
...
// TimeFormat is Sandard MySQL datetime format
...
// ParseTime returns parses string datetime in TimeFormat using loc location.
// Converts MySQL zero to time.Time zero.
...
// DurationString returns d as string in MySQL format Convert time.Duration to string representation of mysql.TIME
