...
const (
    DefaultKey  = "github.com/owner888/kaligo/sessions"
...
func SessionsMany(names []string, store Store) kaligo.HandlerFunc {
    return func(c *kaligo.Context) {
        sessions := make(map[string]Session, len(names))
        for _, name := range names {
            sessions[name] = &session{name, c.Request, store, nil, false, c.ResponseWriter}
        }
...
func Default(c *kaligo.Context) Session {
    return c.MustGet(DefaultKey).(Session)
}

// shortcut to get session with given name
func DefaultMany(c *kaligo.Context, name string) Session {
